...
function renderInterview() {
  ...
  ${iq('Advanced','How would you scale this backend?',`<p>Make the app stateless and run multiple instances behind a load balancer (horizontal scale); add read replicas (extra copies of the database that serve read queries) and connection pooling (reuse a fixed set of open database connections instead of opening a new one each time); cache data that is read often; and move slow work to a background queue. Measure first, then scale the actual bottleneck.</p>`)}
  ${iq('Advanced','What would you cache, and where?',`<p>Cache expensive, frequently-read, rarely-changing data (e.g. restaurant listings) in an in-memory store like Redis, with sensible TTLs and invalidation on writes. Also use HTTP/CDN caching for static responses. Watch out for stale data, and for cache stampedes &mdash; when a popular cached item expires and many requests all rush to rebuild it at once.</p>`)}
  ${iq('Advanced','How do you decide what to index?',`<p>Index columns you filter, join or sort on frequently, especially foreign keys. Weigh the read speedup against slower writes and storage, use composite (multi-column) indexes for filters on several columns &mdash; but note such an index only helps when you filter on its columns from the left, so an index on <code class="inl">(a, b)</code> helps a query on <code class="inl">a</code> but not one on <code class="inl">b</code> alone. Confirm your choices with the query plan.</p>`)}
  ...
  ${iq('Advanced','How would you add rate limiting?',`<p>Cap how many requests each client or API key can make in a time window. Two common methods are the token bucket (each client has a bucket of tokens that refills over time; one request spends one token) and the fixed window (count requests per fixed period, e.g. per minute). Enforce it at the gateway/reverse proxy or with a shared store like Redis, and return <code class="inl">429</code> when the limit is exceeded. It protects the service from abuse and overload.</p>`)}

  <h3 class="section-h" style="margin-top:26px">Quality &amp; delivery</h3>
  ${iq('Intermediate','How did you test it?',`<p>Unit tests for business logic, API/integration tests for endpoints (with a test database and overridden dependencies), and a few end-to-end checks &mdash; following the test pyramid (many fast, cheap unit tests at the bottom, fewer slow end-to-end tests at the top). Tests run in CI on every push.</p>`)}
  ...
  ${iq('Intermediate','How do you manage configuration and secrets?',`<p>Keep all configuration out of the code: read it from environment variables set per environment, and keep secrets in a secret store (never in the code or the image). Promote the same built artifact across dev, staging and production. (This approach is often called "twelve-factor".)</p>`)}
  ${iq('Advanced','How do you monitor it in production?',`<p>Structured logs, metrics (latency, error rate, throughput), and traces, shipped to a dashboard; health checks for liveness/readiness; and alerts that fire when you miss your reliability targets (your SLOs), so problems are caught before users report them.</p>`)}
