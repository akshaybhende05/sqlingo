...
function renderInterview() {
  ...
  ${iq('Beginner','List vs tuple?',`<p>A <code class="inl">list</code> is mutable (you can change it after making it). A <code class="inl">tuple</code> is immutable (fixed once made), which also lets you use it as a dictionary key or put it in a set. Use a list for a collection that grows, and a tuple for a fixed set of values that should not change.</p>`)}
  ${iq('Beginner','Mutable vs immutable types, and the default-argument trap?',`<p>Immutable: <code class="inl">int, float, str, bool, tuple, frozenset</code>. Mutable: <code class="inl">list, dict, set</code>. A default value is created once, when the function is defined &mdash; not each time you call it. So a mutable default (like a list) is quietly shared across every call, which causes bugs:</p><pre class="code">def add(x, bucket=None):   # not bucket=[]
  ...
  ${iq('Beginner','is vs == ?',`<p><code class="inl">==</code> asks "are these values equal?"; <code class="inl">is</code> asks "are these the exact same object in memory?". Use <code class="inl">==</code> for comparing values, and <code class="inl">is</code> only for <code class="inl">None</code>. Do not use <code class="inl">is</code> to compare numbers or strings: Python sometimes reuses the same object for them behind the scenes, so <code class="inl">is</code> can appear to work and then fail unexpectedly.</p>`)}
  ${iq('Beginner','Why are strings immutable, and what does that imply?',`<p>A <code class="inl">str</code> can't be changed in place &mdash; "modifying" one actually makes a new string. That is why strings are safe to share and can be used as dictionary keys. The downside: building a big string by adding to it in a loop (<code class="inl">s += ...</code>) rebuilds the whole string each time, which gets slow. Use <code class="inl">"".join(parts)</code> instead.</p>`)}
...
uniq    = {w.lower() for w in words}</pre>`)}
  ...
  ${iq('Advanced','How does inheritance and the MRO / super() work?',`<p>When a class has more than one parent, Python needs a fixed order in which to look for a method. That order is the <b>MRO</b> (Method Resolution Order), which you can see with <code class="inl">Cls.__mro__</code>. <code class="inl">super()</code> follows the MRO to call the next class in line, which is what makes multiple inheritance and mixins behave predictably.</p>`)}
  ...
  ${iq('Advanced','What do async and await mean?',`<p>An <code class="inl">async def</code> function is a coroutine &mdash; a function that can pause and resume. <code class="inl">await</code> pauses it while it waits for something slow (like a network call), which frees Python to run other tasks in the meantime. Great for handling many waiting tasks at once, not for heavy calculation.</p>`)}
  ...
  ${iq('Intermediate','How should you build a big string efficiently?',`<p>Collect the parts in a list and join them once with <code class="inl">"".join(parts)</code>. Repeatedly doing <code class="inl">+=</code> rebuilds the whole string every time, so it gets slower as the string grows; join does it in a single pass.</p>`)}
