...
import { entries, bySlug } from "../../../lib/python-reference";
...
export function generateMetadata({ params }) {
  ...
  if (!e) return {};
  const title = `${e.name} in Python — syntax and example | CareerLadder`;
  const description = `${e.name}: ${e.summary}`.slice(0, 155);
  const url = `${SITE_URL}/python-reference/${e.slug}`;
  return {
    title,
    description,
    alternates: { canonical: `/python-reference/${e.slug}` },
    openGraph: { title, description, url, type: "article", siteName: "CareerLadder" },
    twitter: { card: "summary", title, description },
  };
}

export default function PythonReferenceTopic({ params }) {
  const e = bySlug[params.topic];
  if (!e) notFound();
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Python reference", item: `${SITE_URL}/python-reference` },
      { "@type": "ListItem", position: 3, name: e.name, item: `${SITE_URL}/python-reference/${e.slug}` },
    ],
  };
      ...
      <main className="hub-main">
        <article className="explain-article">
          <nav className="reading-crumb" aria-label="Breadcrumb">
            <Link href="/python-reference">Python reference</Link> <span>›</span> <span>{e.category}</span>
          </nav>
          ...
          <p className="explain-course">
            Want to run it? <Link href="/courses/dev-python">Practise Python live in the course →</Link>
          </p>
          ...
          {related.length > 0 && (
            <section className="explain-related">
              ...
              <ul>
                {related.map((r) => (
                  <li key={r.slug}><Link href={`/python-reference/${r.slug}`}>{r.name}</Link></li>
                ))}
              </ul>
            </section>
          )}

          <p className="explain-back"><Link href="/python-reference">← All Python reference</Link></p>
        </article>
      </main>
      <SiteFooter />
    </>
