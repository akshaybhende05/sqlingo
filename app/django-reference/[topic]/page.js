...
import { entries, bySlug } from "../../../lib/django-reference";
...
export function generateMetadata({ params }) {
  ...
  if (!e) return {};
  const title = `${e.name} in Django — explained with an example | CareerLadder`;
  const description = `${e.name}: ${e.summary}`.slice(0, 155);
  const url = `${SITE_URL}/django-reference/${e.slug}`;
  return {
    title,
    description,
    alternates: { canonical: `/django-reference/${e.slug}` },
    openGraph: { title, description, url, type: "article", siteName: "CareerLadder" },
    twitter: { card: "summary", title, description },
  };
}

export default function DjangoReferenceTopic({ params }) {
  const e = bySlug[params.topic];
  if (!e) notFound();
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Django reference", item: `${SITE_URL}/django-reference` },
      { "@type": "ListItem", position: 3, name: e.name, item: `${SITE_URL}/django-reference/${e.slug}` },
    ],
  };
      ...
      <main className="hub-main">
        <article className="explain-article">
          <nav className="reading-crumb" aria-label="Breadcrumb">
            <Link href="/django-reference">Django reference</Link> <span>›</span> <span>{e.category}</span>
          </nav>
          ...
          <p className="explain-course">
            Want to build with it? <Link href="/courses/dev-django">Take the Django course →</Link>
          </p>
          ...
          {related.length > 0 && (
            <section className="explain-related">
              ...
              <ul>
                {related.map((r) => (
                  <li key={r.slug}><Link href={`/django-reference/${r.slug}`}>{r.name}</Link></li>
                ))}
              </ul>
            </section>
          )}

          <p className="explain-back"><Link href="/django-reference">← All Django reference</Link></p>
        </article>
      </main>
      <SiteFooter />
    </>
